...
describe('DomainDate', () => {
  ...
  describe('Errors', () => {
    it('should throw validation error if date is invalid', () => {
      ...
      expect(() => DomainDate.create(invalidDate)).toThrow(/date must be a valid date/);
    });
    ...
    it('should throw validation error if value is not a Date instance', () => {
      ...
      // @ts-expect-error testing invalid input
      expect(() => DomainDate.create('2024-01-15')).toThrow(/date must be a valid date/);
    });
    ...
    it('should throw validation error if value is a number', () => {
      ...
      // @ts-expect-error testing invalid input
      expect(() => DomainDate.create(1234567890)).toThrow(/date must be a valid date/);
    });
  });
});
